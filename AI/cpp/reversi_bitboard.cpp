...
std::vector<int> ReversiBitboard::get_flipped_indices(int move_bit) const {
    std::vector<int> flipped_indices;
    if (move_bit < 0 || move_bit >= BOARD_SIZE) {
    ...
    uint64_t flip_mask = _calculate_flips(move_bit, player_board, enemy_board);
